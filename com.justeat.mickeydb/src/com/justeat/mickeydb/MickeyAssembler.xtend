...
class MickeyAssembler {
	...
	def MickeyModel assemble(Resource resource) {
		return assemble(resource.resourceSet.resources.filter[it.URI.toString.endsWith(".mickey")].map[it.allContents.head as MickeyFile], null)
	}
	
	def MickeyModel assemble(Resource resource, MigrationBlock upToMigration) {
		return assemble(resource.resourceSet.resources.filter[it.URI.toString.endsWith(".mickey")].map[it.allContents.head as MickeyFile], upToMigration)
	}
		
	def MickeyModel assemble(Iterable<MickeyFile> files) {
		return assemble(files, null)
	}
	
	def MickeyModel assemble(Iterable<MickeyFile> files, MigrationBlock upToMigration) {
